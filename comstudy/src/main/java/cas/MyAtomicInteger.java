...
public class MyAtomicInteger extends Number implements java.io.Serializable {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        service.execute(()->{
            while(true){
                if(atomicInteger.val>atomicInteger.value) {
                    System.out.println("show:" + atomicInteger.value+","+atomicInteger.val);
                }
            }
        });
        for(int i=0;i<20;i++)
        {
            service.execute(()->{
                for(int j=0;j<10000;j++) {
                    atomicInteger.val = atomicInteger.incrementAndGet();
               //     System.out.println("increase:"+atomicInteger.addAndGet(1));//单用cas可以保证原子性
                }
            });
        }
