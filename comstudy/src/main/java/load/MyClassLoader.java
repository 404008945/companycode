...
public class MyClassLoader extends ClassLoader{
    ...
    @Override
    protected Class<MyClassLoaderTest> loadClass(String name, boolean resolve) throws ClassNotFoundException {
        Class<?> loadedClass = findLoadedClass(name);
        // 需要我自己去加载
        if (loadedClass==null && myselfLoader.contains(name)) {
            loadedClass = findClass(name);
            if (loadedClass!=null){
                return (Class<MyClassLoaderTest>)loadedClass;
            }
        }
        return (Class<MyClassLoaderTest>)super.loadClass(name, resolve);
    }
